...
function Game(props) {
    ...
    useEffect(() => {
        const valueX = prompt('enter colums: ')
        const valueY = prompt('enter rows: ')
    ...
    useEffect(() => {
        ...
        if (valX) {
            ...
            array1.fill(<img className='img' src={food} alt='food' />)
            gh(array1)
        }
    }, [valX])
    ...
    if (xCord.length > 0 && tracker.current === xCord.length) {
        setTimeout(() => {
            alert(`Game over, You completed gamme in ${score.current} moves`)
            props.endGame()
        })
    }
